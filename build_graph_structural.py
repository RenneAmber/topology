...
from savers import save_activations, save_checkpoint, save_losses, save_weights, save_badj
from loaders import *
from graph import *
...
for epoch in args.epochs:
    ...
    if args.dataset == 'cifar10' or args.dataset == 'cifar10_gray':
    ...
    print("node layer list is:",node_layer)  
    ...
    weights = update_weights_to_robust_scale(weights,0.96)    
    ...
    for name in layer_info_dict:
        ...
        if info['name'] == 'fc':    # simply copy the weights
            ...
            weight = np.transpose(info['weights'])
            subA[0:weight.shape[0],0:weight.shape[1]] = weight
            print("subA shape for FC", info['weights'].shape)
            ...
            print("conv 2d weight shape", weights.shape)
            ...
            '''
            # version 2
            print("subA shape for conv 2d", subA.shape)
    ...
    if not args.split: 
        ...
        save_badj(THRESHOLDS, adj, SAVE_DIR, epoch, args.trial)
    else:
